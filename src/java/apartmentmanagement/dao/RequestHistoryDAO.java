...
public class RequestHistoryDAO extends DBContext {
    ...
    public int insert(RequestHistory history) {
        // Ghi created_at = lúc gửi (tránh DEFAULT SYSUTCDATETIME → lệch ~7h so với giờ VN)
    ...
    /** Fallback khi DB không cho set created_at tường minh. */
    private int insertLegacy(RequestHistory history) {
        ...
        try {
            ...
            if (history.getCreatedAt() == null) {
                history.setCreatedAt(DateTimeUtil.nowTimestamp());
            }
            int affected = statement.executeUpdate();
            if (affected > 0) {
                resultSet = statement.getGeneratedKeys();
