/*
================================================================================
  Seed data demo - ApartmentManagement (TV1)
  Chạy SAU schema.sql:
  ...
  Occupancy (ACTIVE): unit%3=0 VACANT · unit lẻ OWNED · unit chẵn RENTED.
  INACTIVE: loại hình UI = N/A (DB vẫn lưu occupancy_type).
...
/*--------------------------------------------------------------------------------
  ...
  2) Bulk: mọi tầng × 6 căn (xx01–xx06), trừ 5 mã demo
     ...
     Occupancy (khi ACTIVE):
       unit % 3 = 0 → VACANT (sẵn sàng, chưa có cư dân)
       unit lẻ     → OWNED
       unit chẵn   → RENTED
...
(5, N'A-1201', 1, N'A', 12, 75.00, N'RENTED', N'ACTIVE', N'Cho thuê – chưa gán cư dân active');
...
SELECT
    ...
    /* VACANT trước (unit 3,6); còn lại lẻ OWNED / chẵn RENTED */
    CASE
        WHEN u.un % 3 = 0 THEN N'VACANT'
        WHEN u.un % 2 = 1 THEN N'OWNED'
        ELSE N'RENTED'
    ...
    CASE
        WHEN (f.fl * 7 + u.un * 11 + b.building_id * 3) % 5 = 0 THEN
            ...
            END
        WHEN u.un % 3 = 0 THEN N'Sẵn sàng bàn giao – chưa có cư dân chuyển vào'
        ELSE NULL
    END AS notes
FROM bld b
