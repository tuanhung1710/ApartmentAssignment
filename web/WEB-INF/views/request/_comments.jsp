...
(function () {
    ...
    /** Đồng hồ HH:mm realtime (giờ máy client – thường = VN). */
    function pad2(n) {
    ...
    /** Gỡ 1 bubble optimistic (chờ server) – tin mình vừa gửi. */
    ...
    function appendComment(c) {
        ...
        // commentId có thể là số server hoặc local-xxx (optimistic)
        var cid = c.commentId;
        ...
        // Tin server của mình → thay bubble optimistic, GIỮ HH:mm lúc bấm Gửi (realtime)
        var mine = !!c.mine || (c.userId === CURRENT_USER_ID);
        var pendingTime = null;
        if (mine && !c.pending) {
            var firstPending = timeline.querySelector('.comment-row.mine[data-pending="1"] .comment-time');
            if (firstPending) {
                pendingTime = firstPending.getAttribute('data-hm') || firstPending.textContent;
            }
        ...
        // Ưu tiên: giờ máy lúc gửi (optimistic) → server → now
        var timeLabel = pendingTime
            || ((c.createdAt && String(c.createdAt).trim()) ? String(c.createdAt).trim() : '')
            || formatHm(new Date());
        ...
        meta.innerHTML = '<strong>' + escapeHtml(c.userFullName || (mine ? CURRENT_USER_NAME : '')) + '</strong>'
            + ((c.userRole || (mine ? CURRENT_USER_ROLE : ''))
                ...
                : '')
            + '<span class="text-muted small ms-1 comment-time" data-hm="' + escapeHtml(hmDisplay)
            + '" title="' + escapeHtml(timeLabel) + '">'
    ...
    /** Hiện ngay bubble của mình với HH:mm lúc bấm Gửi. */
    ...
    function sendViaHttp(html, btn) {
        ...
        return fetch(COMMENT_API, {
            ...
            // Đã clear editor + optimistic bubble; confirm bằng comment server
            if (data.comment) {
                // Giữ HH:mm lúc gửi nếu server không trả createdAt
